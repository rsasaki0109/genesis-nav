...
![genesis-nav smoke scenario on real Genesis](docs/media/genesis_smoke.gif)

A diff-drive robot (blue) navigating to its goal (green) on real Genesis 1.0,
its trajectory traced in orange, recorded headless on GPU (`gs.cuda`):

```bash
gnav run examples/scenarios/smoke.yaml --fast --backend genesis
# record the GIF above (needs a Genesis install + ffmpeg):
python scripts/record_genesis_demo.py --scenario examples/scenarios/smoke.yaml --out-dir /tmp/gsframes
```

The same scenario also runs without Genesis on the deterministic fallback
integrator (`--backend fallback`, the default), and mirrors to ROS 2 with
`--ros`:
...
The hero GIF above is captured by `scripts/record_genesis_demo.py` (real
Genesis + ffmpeg). A separate terminal-tour GIF is regenerated by
`make demo-gif` (requires `asciinema` and `agg`); see `docs/demo_script.md`
